...
static const CGFloat kMinBadgeWidth = 22.0;              // The minimum badge width for each item (default 22.0).
static const CGFloat kBadgeHeight = 14.0;                // The badge height for each item (default 14.0).
static const CGFloat kRowRightMargin = 5.0;              // The spacing between the right edge of the badge and the edge of the table column.
static const CGFloat kIconSpacing = 2.0;                 // The spacing between the icon and it's adjacent cell.
static const CGFloat kDisclosureTriangleSpace = 18.0;   // The indentation reserved for disclosure triangles for non-group items.
...
{
    if( self.delegateDataSourceProxy.delegate != delegate )
    {
        self.delegateDataSourceProxy.delegate = delegate;
...
{
    if( self.delegateDataSourceProxy.dataSource != dataSource )
    {
        self.delegateDataSourceProxy.dataSource = dataSource;
...
{
    if( !NSEqualSizes( _iconSize, newIconSize ) )
    {
...
- (void) reloadData
{
    ...
    if( self.numberOfSelectedRows > 0 )
    {
        ...
        NSInteger firstSelectedRow = (NSInteger)selectedIndexes.firstIndex;
        ...
        if( [self isGroupItem:[self itemAtRow:firstSelectedRow]] )
        {
            ...
            BOOL foundRow = NO;
            for( NSInteger i = firstSelectedRow; i > 0; --i )
            {
                if( ![self isGroupItem:[self itemAtRow:i]] )
                {
                    [self selectRowIndexes:[NSIndexSet indexSetWithIndex:(NSUInteger)i] byExtendingSelection:NO];
                    foundRow = YES;
                    break;
                }
            }

            //If there is no non-group row preceding the currently selected group item, remove the selection from the Source List
...
- (NSUInteger) numberOfGroups
{
    ...
    if( [self.delegateDataSourceProxy respondsToSelector:@selector(sourceList:numberOfChildrenOfItem:)] )
        number = [self.delegateDataSourceProxy sourceList:self numberOfChildrenOfItem:nil];
...
- (BOOL) isGroupAlwaysExpanded:(id) group
{
    ...
    if( [self isGroupItem:group])
    {
        ...
        {
            isExpanded = [self.delegateDataSourceProxy sourceList:self isGroupAlwaysExpanded:group];
        }
    }
...
- (BOOL) itemHasBadge:(id) item
{
    ...
    if( !self.isViewBasedSourceList )
    {
        ...
        {
            hasBadge = [self.delegateDataSourceProxy sourceList:self itemHasBadge:item];
        }
    }
...
- (void) selectRowIndexes:(NSIndexSet*) indexes byExtendingSelection:(BOOL) extend
{
    ...
    if( self.allowsEmptySelection == NO && numberOfIndexes == 0 )
    {
        return;
    }
    ...
    {
        if( ![self isGroupItem:[self itemAtRow:(NSInteger)selectedIndexes[i]]] )
        {
...
- (NSRect) frameOfOutlineCellAtRow:(NSInteger) row
{
    ...
    NSRect frame = [self isGroupAlwaysExpanded:[self itemAtRow:row]] ? NSZeroRect : [super frameOfOutlineCellAtRow:row];
...
- (NSRect) frameOfCellAtColumn:(NSInteger) column row:(NSInteger) row
{
    ...
    if( [self isGroupItem:item] )
    {
        ...
        return NSMakeRect( minX, NSMidY( cellFrame ) - (cellSize.height * 0.5), NSWidth( rowRect ) - minX, cellSize.height );
    }
    else
    {
        CGFloat leftIndent = [self levelForRow:row]*[self indentationPerLevel] + kDisclosureTriangleSpace;

        // Calculate space left for a badge if need be

        CGFloat rightIndent = [self sizeOfBadgeAtRow:row].width + kRowRightMargin;
        ...
        if( ![self isGroupItem:item] && [self.delegateDataSourceProxy respondsToSelector:@selector(sourceList:itemHasIcon:)] )
        {
            ...
            {
                leftIndent += self.iconSize.width + (kIconSpacing * 2.0);
            }
        }

        return NSMakeRect( leftIndent, NSMidY(rowRect) - (cellSize.height * 0.5), NSWidth(rowRect) - rightIndent - leftIndent, cellSize.height );
    }
}
...
- (void) drawBadgeForRow:(NSInteger) rowIndex inRect:(NSRect) badgeFrame
{
    ...
    self.reusableBadgeCell.badgeValue = (NSUInteger)[self badgeValueForItem:rowItem];
    ...
    self.reusableBadgeCell.highlighted = [self.selectedRowIndexes containsIndex:(NSUInteger)rowIndex];
        ...
        if( keyCharacters.length > 0 )
        {
            ...
            if( firstKey == NSUpArrowFunctionKey || firstKey == NSDownArrowFunctionKey )
            {
                ...
                if( [selectedIndexes count] == 1 && !(theEvent.modifierFlags & NSShiftKeyMask) )
                {
                    ...
                    [self selectRowIndexes:[NSIndexSet indexSetWithIndex:(NSUInteger)newRow] byExtendingSelection:NO];
                    return;
                }
            }
